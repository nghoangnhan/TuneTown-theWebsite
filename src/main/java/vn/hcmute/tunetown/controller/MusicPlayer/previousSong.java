...
@WebServlet(urlPatterns = {"/previousSong"})
public class previousSong extends HttpServlet {
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            if (Queue.currentSongIndex - 1 >= 0) {
                Queue.currentSongIndex--;
            }
            else {
                Queue.currentSongIndex = Queue.getQueue().size() - 1;
            }
